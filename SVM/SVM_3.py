...
def SVM(X_train, labels_train, X_test, labels_test):
    clf = svm.SVC()
    clf.fit(X_train, labels_train)
    pred = clf.predict(X_test)
    accuracy = cluster_acc(pred, labels_test)
    print('SVM:{:>6.4f}'.format(accuracy))
    return accuracy
...
def draw():
    ...
    plt.bar(index + width * 3, SVM_data, width, label="SVM", color="teal")
    ...
    for a, b in zip(index + width*2, LR_data):
        plt.text(a, b, '%.4f' % b, ha='center', va='bottom', fontsize=7);
    for a, b in zip(index + width*3, SVM_data):
        plt.text(a, b, '%.4f' % b, ha='center', va='bottom', fontsize=7);
...
if __name__ == '__main__':
    ...
    for i in range(1, 4):
        ...
        acc_data = SVM(X_train, labels_train, X_test, labels_test)
        SVM_data.append(acc_data)
